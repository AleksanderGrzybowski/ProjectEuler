    ...
    public Integer getCalculatedSolution() {
        ...
        for (int row = 0; row < SIZE; ++row) {
            for (int col = 0; col < SIZE - 3; ++col) {
                sequences.add(asList(GRID[row][col], GRID[row][col + 1], GRID[row][col + 2], GRID[row][col + 3]));
            }
        }
        ...
        for (int col = 0; col < SIZE; ++col) {
            for (int row = 0; row < SIZE - 3; ++row) {
                sequences.add(asList(GRID[row][col], GRID[row + 1][col], GRID[row + 2][col], GRID[row + 3][col]));
            }
        }
        ...
        for (int row = 0; row < SIZE - 3; ++row) {
            for (int col = 0; col < SIZE - 3; ++col) {
                sequences.add(asList(GRID[row][col], GRID[row + 1][col + 1], GRID[row + 2][col + 2], GRID[row + 3][col + 3]));
            }
        }
